    ...
    <div class="stock-filter">
        ...
        <form method="get" action="">
          <select name="product-type">
            <option value="all"><?php _e('All Product Types','stock-manager'); ?></option>
          ...
          <select name="manage-stock">
            <option value="all"><?php _e('Yes/No manage stock','stock-manager'); ?></option>
          ...
          <select name="stock-status">
            <option value="all"><?php _e('In/out stock','stock-manager'); ?></option>
          ...
          <input type="submit" name="show-product-filter" value="<?php _e('Show','stock-manager'); ?>" class="btn btn-info" />
          <a href="<?php echo admin_url().'admin.php?page=stock-manager'; ?>" class="btn btn-danger"><?php _e('Clear filter','stock-manager'); ?></a>
      </form>    
    </div> 
    <div>
      <form>
        <!--*************************************Add text field for input sku number**************************************************************************************************-->
          <p>Search By Sku: </p>
          <input type="text" name="sku" placeholder="sku"/>

          <input type="hidden" name="page" value="stock-manager" />
          <input type="submit" name="show-product-sku" value="<?php _e('Show','stock-manager'); ?>" class="btn btn-info" />
        </form>
    ...
    <form method="post" action="" style="position:relative;">
      <div class="lineloader"></div>  
        <table class="table-bordered">
          <tr>
            <th><?php _e('Img','stock-manager'); ?></th>
            ...
            <th style="width:50px;"><?php _e('Stock Adjustment','stock-manager'); ?></th>
            <th style="width:100px;"><?php _e('Save','stock-manager'); ?></th>
          </tr>

        <!--***************************************IMPORTANT to display the product****************************************************************************************************--> 
        
        <?php $products = $stock->get_products(); ?>
        <?php foreach( $products as $item ){ 
          ...
          if(!empty($product_meta['_invlocation_field'][0])){
            $invlocation=$product_meta['_invlocation_field'][0];
          }else{
            $invlocation='';
          }
        ?>
        ...
        <!--**************************************************************************************************************************************************************************-->
          ...
          <tr>
            ...
            <td><?php echo $item_product->get_image(array(40,40));?></td>
            
            <td><span name="sku[<?php echo $item->ID;?>]" class="sku_<?php echo $item->ID; ?>" data-sku="<?php echo $sku;?>" ><?php echo $sku; ?></span></td>


            <td class="td_center"><?php echo $item->ID; ?></td>

            <td><span name="title[<?php echo $item->ID;?>]" class="title_<?php echo $item->ID; ?>" data-title="<?php echo $item->post_title;?>" ><?php echo $item->post_title; ?></td>
            ...
            <td style="width:100%;">
              <span><input type="text" name="invlocation[<?php echo $item->ID; ?>]" class="invlocation_<?php echo $item->ID; ?>" value="<?php echo $invlocation;?>" style="width:100%"></span>
            </td>
<!--             <td class="td_center adjust" contenteditable="true">
              <?php echo $invlocation;?>
            </td> -->
            <?php 
              $class = '';
            ...
            <td class="td_center <?php echo $class; ?>" style="width:90px;">
              <input type="number" name="stock[<?php echo $item->ID; ?>]" step="1" value="<?php echo $stock_number; ?>" class="stock_<?php echo $item->ID; ?>" style="width:90px;" readonly/>
            </td>

            <td class="td_center adjust" style="width:90px;">
              <input type="number" name="stockadjust[<?php echo $item->ID; ?>]" step="1" class="stockadjust_<?php echo $item->ID; ?>"style="width:90px;" />
            </td>
        ...
        ?>
          <tr class="variation-line variation-item-<?php echo $item->ID; ?>">
            ...
            <td><span name="sku[<?php echo $vars->ID;?>]" class="sku_<?php echo $vars->ID; ?>" data-sku="<?php echo $sku;?>" ><?php echo $sku; ?></span></td>


            <td class="td_center"><?php echo $vars->ID; ?></td>
            
            <?php
              $attrs=$item_product->get_variation_attributes();
              $title_output='';
              foreach($attrs as $k=>$v){
                  $title_output.=$v.' ';
              }
              $title_output=$item->post_title.'-'.$title_output;
            ?>

            <td><span name="title[<?php echo $vars->ID;?>]" class="title_<?php echo $vars->ID; ?>" data-title="<?php echo $title_output;?>" ><?php echo $title_output; ?></td>
            ...
            <td style="width:100%;">
              <span><input type="text" name="invlocation[<?php echo $vars->ID; ?>]" class="invlocation_<?php echo $vars->ID; ?>" value="<?php echo $invlocation;?>" style="width:100%"></span>
            </td>
            <?php
            $class = '';
              if(!empty($product_meta['_stock'])){
            ...
            <td class="td_center <?php echo $class; ?>" style="width:90px;">
              ...
              <input type="number" name="stock[<?php echo $vars->ID; ?>]" step="1" value="<?php echo $stock_number; ?>" class="stock_<?php echo $vars->ID; ?>" style="width:90px;" readonly/>
            </td>

            <td class="td_center adjust" style="width:90px;">
              <input type="number" name="stockadjust[<?php echo $vars->ID; ?>]" step="1" class="stockadjust_<?php echo $vars->ID; ?>" style="width:90px;" />
            </td>
